...
export class AssetWaitHandler {
  ...
  private calculateBackoffInterval(attempt: number): number {
    const { initialIntervalMs, maxIntervalMs, exponentialBase } = this
      .config as Required<WaitForAssetsConfig>;
  ...
  private async checkAssetStatus(scriptId: string): Promise<{
    ...
    const voice =
      await this.repositories.voicesRepository.findByScriptId(scriptId);
    const images =
      await this.repositories.imagesRepository.findByScriptId(scriptId);
    ...
    const permanentImageFailures = images.filter(
      (img): img is ImageAsset & { status: "FAILED" } =>
    ...
    const isAssetComplete = (
      ...
      status === "COMPLETED" ||
      (status === "FAILED" && this.isTemporaryError(error));
  ...
  private validateAssets(voice: VoiceAsset | null, images: ImageAsset[]) {
    const errors: string[] = [];
    ...
    if (images.length === 0) {
      ...
      if (invalidImages.length > 0) {
        errors.push(
          `Missing image URLs for scenes: ${invalidImages.map((img) => img.scene_index).join(", ")}`
        );
      }
    }
  ...
  async waitForAssets(scriptId: string): Promise<AssetWaitResult> {
    ...
    while (attempts < maxAttempts) {
      const { voice, images, allComplete, hasFailures, errors } =
      ...
      if (allComplete) {
        this.validateAssets(voice, images);
      ...
      if (attempts % 10 === 0) {
        console.log(
          `Still waiting for assets (attempt ${attempts}/${maxAttempts}):`,
          {
