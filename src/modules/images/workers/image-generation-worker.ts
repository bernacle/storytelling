...
export const FREE_SETTINGS = {
  minRequestInterval: 10 * 1000, // 10 seconds between requests
  concurrency: 1, // Process 1 job at a time
  limiterMax: 3, // Allow 3 requests per minute
...
async function enforceRateLimit() {
  ...
  if (timeSinceLastRequest < FREE_SETTINGS.minRequestInterval) {
    const delay = FREE_SETTINGS.minRequestInterval - timeSinceLastRequest;
...
export function createImageWorker(
  ...
  const worker = new Worker<ImageGenerationJob>(
    ...
    async (job: Job) => {
      ...
      try {
        ...
        console.log("Generating image with options:", {
          ...
          maxAttempts: FREE_SETTINGS.maxRetries,
        });
        ...
        if (isRateLimitError(error)) {
          console.log("Rate limit or model busy, retrying shortly...", {
            attempt: job.attemptsMade + 1,
            maxAttempts: FREE_SETTINGS.maxRetries,
            retryDelay: FREE_SETTINGS.retryDelay,
          });

          await new Promise((resolve) =>
            setTimeout(resolve, FREE_SETTINGS.retryDelay)
          );
          throw error;
        }
        ...
        if (job.attemptsMade + 1 >= FREE_SETTINGS.maxRetries) {
    ...
    {
      connection,
      concurrency: FREE_SETTINGS.concurrency,
      removeOnComplete: { count: 100 },
      removeOnFail: { count: 100 },
      prefix: "storytelling",
      limiter: {
        max: FREE_SETTINGS.limiterMax,
        duration: 1000 * 60, // per minute
      },
      settings: {
        backoffStrategy: (attemptsMade: number) => {
          return Math.min(
            FREE_SETTINGS.retryDelay * Math.pow(2, attemptsMade),
            30000
          );
        },
      },
  ...
  worker.on("failed", (job, error) => {
    if (
      ...
      (job?.attemptsMade ?? 0) >= FREE_SETTINGS.maxRetries
    ) {
      console.error(
        `[Worker] Failed job ${job?.id} after ${job?.attemptsMade ?? 0} attempts:`,
        error
      );
    }
  });
